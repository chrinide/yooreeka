...
public class YCrawler extends WebCrawler {
	...
	/**
	 * The location where we will store the fetched data. Note that this is a location for all the crawls
	 * of this class. If you would like to change it use the <tt>setRootDir()</tt> method.
	 */
	...
	private CrawlController setup() {
        ...
        RobotstxtServer robotsTextServer = new RobotstxtServer(robotstxtConfig, pageFetcher);

        CrawlController controller=null;
		try {
			controller = new CrawlController(crawlConfiguration, pageFetcher, robotsTextServer);
		} catch (Exception e) {
			// TODO proper logging ...
			e.printStackTrace();
		} 
	...
	private String getRootDir() {
    	...
    	if ( rootDir == null || rootDir.trim().length() == 0) {
    		...
    		// Create a default location for storing the data, relative to the IWEB2_HOME location
